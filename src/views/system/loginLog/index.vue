...
import { getLoginLogPaginate } from '/@/api/loginLog';
import { LoginLogSearchRequest, LoginLogInfo } from '/@/types/bindings';

// 定义变量内容
const state = reactive({
	tableData: {
		data: [] as LoginLogInfo[],
		total: 0
	},
	loading: false,
	search: {
...
const getTableData = async () => {
	state.loading = true;
	getLoginLogPaginate(state.search).then((res) => {
